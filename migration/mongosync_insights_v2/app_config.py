...
# Developer credits (displayed in credits overlay)
DEVELOPER_CREDITS = {
...
ALLOWED_MIME_TYPES = [
    'application/json',  # JSON log files
...
# Streaming log settings
LOG_FILE_PATH = os.getenv('MI_LOG_FILE_PATH', '')
STREAM_UPDATE_INTERVAL = float(os.getenv('MI_STREAM_UPDATE_INTERVAL', '1.0'))
STREAM_MAX_BUFFER_SIZE = int(os.getenv('MI_STREAM_MAX_BUFFER_SIZE', '1000'))
...
CONNECTION_TIMEOUT_MS = int(os.getenv('MI_TIMEOUT_MS', '30000'))  # Increased to 30 seconds
...
def get_mongo_client(connection_string):
    ...
    Args:
        connection_string (str): MongoDB destination cluster connection string
    ...
    try:
        ...
        client = MongoClient(
            ...
            minPoolSize=1,
            maxIdleTimeMS=600000,  # 10 minutes - allows longer idle periods
            serverSelectionTimeoutMS=CONNECTION_TIMEOUT_MS,
            connectTimeoutMS=CONNECTION_TIMEOUT_MS,
            socketTimeoutMS=CONNECTION_TIMEOUT_MS * 2,  # Double timeout for socket operations
            retryWrites=True,
            retryReads=True
        )
        ...
        logger.error(f"Invalid MongoDB destination cluster connection string: {e}")
        raise
...
def get_database(connection_string, database_name):
    ...
    try:
        ...
        logger = logging.getLogger(__name__)
        logger.warning("MongoDB connection failed, clearing cache and retrying...")
...
def validate_connection(connection_string):
    """
    Validate a MongoDB destination cluster connection string and test connectivity.
    
    Args:
        connection_string (str): MongoDB destination cluster connection string to validate
